...
describe('RecipeEntry', () => {
  ...
  describe('getUserMsg', () => {
    it('returns New tag for recipes created after lastSeen', () => {
      const user = new UserEntry(
        "Test",
        Visibility.private,
        [],
        new Date("2024-03-01"), // lastSeen
        new Date("2024-03-01"),
        "user1"
      );
      const recipe = new RecipeEntry(
        ...
        "user1",
        "id",
        new Date("2024-04-01"), // created after lastSeen
        new Date("2024-04-01"),
        "user1"
      );

      const result = recipe.getUserMsg(user);
      expect(result.props.color).toBe("red");
      expect(result.props.children).toBe("New");
    });

    it('returns Updated tag for recipes updated after lastSeen but created before', () => {
      const user = new UserEntry(
        "Test",
        Visibility.private,
        [],
      ...
      const recipe = new RecipeEntry(
        ...
        "user1",
        "id",
        new Date("2024-01-01"), // created before lastSeen
        new Date("2024-04-01"), // updated after lastSeen
        "user1"
      );

      const result = recipe.getUserMsg(user);
      expect(result.props.color).toBe("orange");
      expect(result.props.children).toBe("Updated");
    });

    it('returns empty div for old recipes', () => {
      const user = new UserEntry(
        "Test",
        Visibility.private,
        [],
        new Date("2024-06-01"), // lastSeen
        new Date("2024-06-01"),
        "user1"
      );
      const recipe = new RecipeEntry(
        ...
        "id",
        new Date("2024-01-01"), // created before lastSeen
        new Date("2024-02-01"), // updated before lastSeen
        "user1"
      );
